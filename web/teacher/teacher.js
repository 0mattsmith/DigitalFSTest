...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // Auto-load if there's a class in the URL hash (e.g. shared bookmark).
...
async function loadClass(code) {
  ...
  // Lazy-load the Firebase SDK from CDN.
  ...
  const q = fsMod.query(
    fsMod.collection(db, 'attempts'),
    fsMod.where('classCode', '==', code),
    fsMod.orderBy('finishedAt', 'desc'),
    fsMod.limit(500),
  );
  if (unsubscribe) unsubscribe();
  unsubscribe = fsMod.onSnapshot(q, (snap) => {
    const items = [];
...
function renderAll() {
  $('#class-stats').textContent =
    currentItems.length + ' attempt' + (currentItems.length === 1 ? '' : 's') +
    ' · ' + new Set(currentItems.map(i => i.studentName)).size + ' student' +
    (new Set(currentItems.map(i => i.studentName)).size === 1 ? '' : 's');
  renderLeaderboard();
  renderWeakTopics();
}
...
function renderWeakTopics() {
  // Roll up areaStats from every attempt in the class.
  const agg = {};
...
function exportCsv() {
  ...
  const cols = ['finishedAt', 'studentName', 'level', 'totalScore', 'totalMax', 'pct', 'grade', 'mode', 'seed', 'registration'];
  ...
  a.download = 'dfsq-class-' + currentClassCode + '-' + new Date().toISOString().slice(0, 10) + '.csv';
